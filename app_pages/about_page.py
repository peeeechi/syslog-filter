...
def run():
    ...
    * **単一コマンド起動**: `streamlit run main_app.py` という一つのコマンドで全ての機能にアクセスできます。
    ...
    * **結果のダウンロード**: フィルタリングされたログデータをCSV形式でダウンロードできます。
    ...
    #### 「日付/時刻抽出」ページ
    * **日付による抽出**: ログデータから検出された**利用可能な日付の一覧**から、抽出したい日付を簡単に選択できます。
    * **時刻範囲による抽出**: **5分刻みで生成された時刻リスト**から、開始時刻と終了時刻を選択し、指定した時間範囲内のログを抽出できます。
    * **表示行数のカスタマイズ**: 結果表示の最大行数をスライダーで調整できます。
